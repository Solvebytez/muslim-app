...
Notifications.setNotificationHandler({
  handleNotification: async (notification) => {
    const { soundEnabled, prayerName, isAzanNotification, notificationType } =
...
export const useUnifiedPrayerNotifications = (
  ...
  const generateNotificationId = (
    ...
    return `${type}-${prayerName}-${date}-${time}`
      .toLowerCase()
      .replace(/[^a-z0-9-]/g, "");
  };
  ...
  const saveScheduledNotifications = async (
    ...
    try {
      ...
      console.log(
        `💾 Saved ${notifications.length} notifications to AsyncStorage`
      );
    } catch (error) {
  ...
  > => {
    try {
      ...
      if (stored) {
        ...
        scheduledNotificationsCache = notifications;
        console.log(
          `📱 Loaded ${notifications.length} notifications from AsyncStorage`
        );
  ...
  const scheduleUnifiedPrayerNotifications = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        `📅 Scheduling optimized notifications for ${daysToSchedule} days`
      );
      ...
      for (let dayOffset = 0; dayOffset < daysToSchedule; dayOffset++) {
        ...
        for (const prayer of prayerTimes) {
          ...
          const azanNotificationId = generateNotificationId(
            prayer.name,
            dateStr,
            prayer.originalTime,
            "azan"
      ...
      console.log(
        `✅ Scheduled ${totalScheduled} total notifications (optimized for storage)`
      );
    } catch (error) {
  ...
  const scheduleIndividualNotification = async (
    ...
    try {
      ...
      console.log(
        `✅ Scheduled ${prayer.name} azan in ${hoursUntil}h ${minutesUntil}m - Sound: ${soundEnabled}`
      );
    } catch (error) {
  ...
  const scheduleReminderNotification = async (
    ...
    try {
      ...
      const trigger =
        Platform.OS === "android"
          ? { ...timeIntervalTrigger, channelId: "prayer-reminders" }
          : timeIntervalTrigger;
  ...
  useEffect(() => {
    const subscription = Notifications.addNotificationReceivedListener(
      ({ request }) => {
        ...
        console.log(
          `📱 Notification received: ${prayerName} (${notificationType})`
        );
  ...
  useEffect(() => {
    const subscription = Notifications.addNotificationResponseReceivedListener(
      ({ notification }) => {
        ...
        console.log(
          `👆 Notification tapped: ${prayerName} (${notificationType})`
        );
  ...
  const listAllScheduledNotifications = useCallback(async () => {
    try {
      ...
      const reminderNotifs = notifications.filter(
        (n) => n.content.data?.notificationType === "reminder"
      );
      const azanNotifs = notifications.filter(
        (n) => n.content.data?.notificationType === "azan"
      );
      ...
      notifications.forEach((notif, index) => {
        ...
        console.log(
          `${index + 1}. ${data.prayerName || "Unknown"} (${type}) - ${
            notif.content.title
          }`
        );
      });
